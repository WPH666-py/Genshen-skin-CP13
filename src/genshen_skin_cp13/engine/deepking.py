...
def pkg_root():
    """包目录 .../genshen_skin_cp13(本模块在它的 engine/ 下, 故向上一层)。
...
# src/client/genshen-cp13.module.css 完全一致的取值, 保证在线转换与离线生成一致。
...
FALLBACK_CSS = """:root {
  ...
  --bg-layer-2: #f6ecdd;
  --bg-layer-3: #ead9c2;
  --label-primary: #2b1f16;
  --label-secondary: #5c4632;
  --label-tertiary: #8d7458;
  --brand-primary: #c9a43f;
  --accent: #c9a43f;
  --primary: #c9a43f;
  --interactive-bg-hover-solid: #f4e6cf;
  --interactive-bg-active: #e8d2ad;
  --border-l1: #ece0cc;
  --border-l2: #d8c3a2;
  --border: #d8c3a2;
}
        ...
        <section class="mock" style="background:%(bg)s;color:%(bgText)s">
          ...
          <div class="body">
            <aside style="background:%(sidebarBg)s;color:%(sidebarText)s;border-color:%(border)s">
              ...
              <div class="item">📁 Genshen-skin-CP13</div>
...
<h2>🌙 暗色 Dark</h2>%(dark)s
        ...
        "light": region(p, "亮色 · 原神CP13", mascot.get("light")),
        "dark": region(d, "暗色 · 原神CP13", mascot.get("dark")),
    }
