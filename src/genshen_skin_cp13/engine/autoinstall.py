...
原神 CP 壁纸套件 13 —— 自动安装器
  ...
  genshen-cp13-install                 # 自动: 装依赖 -> 生成壁纸 -> 设为桌面 -> 注册已装 IDE
  genshen-cp13-install --no-wallpaper  # 只做 IDE 注册, 不动桌面壁纸
  genshen-cp13-install --only vscode   # 只处理指定目标
...
def mcp_stdio_entry():
    ...
    return {
        ...
        "args": ["-m", "genshen_skin_cp13.mcp_server"],
        "env": {},
    }
